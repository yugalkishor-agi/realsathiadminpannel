...
export default function BannerManager() {
  ...
  const upload = async (placement, slot, file) => {
    ...
    if (saveResult.error) setError(saveResult.error.message); else { await supabase.rpc('admin_record_audit', { p_action: 'banner_updated', p_entity_type: 'banner', p_entity_id: `${placement}-${slot}`, p_metadata: { path } }); await load() } setBusy('')
  }
  const remove = async item => { setBusy(`${item.placement}-${item.slot}`); const fileResult = await supabase.storage.from('app-banners').remove([item.image_path]); if (fileResult.error) setError(fileResult.error.message); else { const result = await supabase.from('app_banners').delete().eq('id', item.id); if (result.error) setError(result.error.message); else { await supabase.rpc('admin_record_audit', { p_action: 'banner_removed', p_entity_type: 'banner', p_entity_id: item.id, p_metadata: { path: item.image_path } }); await load() } } setBusy('') }
