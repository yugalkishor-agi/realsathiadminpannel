...
import com.incoteam.realsaathi.data.model.common.ApiErrorResponse
import com.incoteam.realsaathi.data.remote.AuthApiService
import com.incoteam.realsaathi.core.device.DeviceMetadataProvider
import java.io.File
...
class AuthRepository(
    ...
    suspend fun verifyOtp(
        ...
        val subscriberNumber = PhoneNumberFormatter.toIndianSubscriberNumber(phoneNumber)
        val device = appContext?.let(DeviceMetadataProvider::read)
        val request = VerifyOtpRequest(
            ...
            deviceBrand = device?.deviceBrand.orEmpty(),
            brandName = device?.deviceBrand.orEmpty(),
            countryCode = device?.countryCode.orEmpty(),
            appBrand = device?.appBrand ?: "RealSaathi"
        )
    ...
    suspend fun deleteAccount(accessToken: String): Result<Unit> = runCatching {
        executeProtectedRequest(
            ...
            executeWithConfiguredEndpoint(BuildConfig.DELETE_ACCOUNT_URL) { url ->
                authApiService.deleteAccount(url = url, sessionToken = sessionToken(activeToken))
            }
        }
        Unit
    }
    ...
    suspend fun getZegoToken(accessToken: String): Result<ZegoTokenResponse> = runCatching {
        executeProtectedRequest(
            accessToken = accessToken,
            fallbackMessage = "Unable to connect calling right now."
        ) { activeToken ->
            executeWithConfiguredEndpoint(BuildConfig.ZEGO_TOKEN_URL) { url ->
                authApiService.getZegoToken(
                    url = url,
                    sessionToken = sessionToken(activeToken)
                )
            }
    ...
    suspend fun rechargeOrder(accessToken: String, request: RechargeOrderRequest): Result<RechargeOrderResponse> =
        runCatching {
            executeProtectedRequest(
                accessToken = accessToken,
                fallbackMessage = "Unable to process recharge right now."
            ) { activeToken ->
                executeWithConfiguredEndpoint(BuildConfig.RECHARGE_ORDER_URL) { url ->
                    authApiService.rechargeOrder(url, sessionToken(activeToken), request)
                }
            }
        }
    ...
    private fun shouldLogoutOnAccessFailure(errorMessage: String): Boolean {
        return errorMessage.trim().lowercase().contains("session replaced")
    }
